...
export default function MapPage() {
  ...
  useEffect(() => { fetchListings(); }, [loc.selectedLat, loc.selectedLng, loc.radius, loc.category]); // eslint-disable-line
        ...
        <div className="md:hidden absolute top-0 left-0 right-0 z-[100] pointer-events-none p-4 space-y-3">
          <div className="pointer-events-auto relative">
            <div className="flex items-center gap-2">
              <form onSubmit={handleSearchAddress} className="flex-1 flex items-center bg-card/90 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.1)] rounded-2xl px-4 py-3 border border-border/40 focus-within:ring-2 focus-within:ring-primary/30 transition-all">
                <FiSearch className="text-muted-foreground mr-3" />
                <input
                  type="text"
                  placeholder={lang === "bn" ? "কোথায় খুঁজছেন?" : "Where to?"}
                  value={manualAddress}
...
function SidebarContent({ handleSearchAddress, manualAddress, setManualAddress, loc, lang, t, useMyLocation, listings, loading, suggestions, onSelectSuggestion, showSuggestions, clearSearch, isDrawer }) {
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="p-7 border-b border-border bg-card relative z-10 shadow-sm">
          ...
          <div className="relative mb-6 group">
            <form onSubmit={handleSearchAddress} className="relative flex items-center">
              <div className="absolute left-4 text-muted-foreground pointer-events-none group-focus-within:text-primary transition-colors">
                 <FiSearch size={18} />
              </div>
              <input
                type="text"
