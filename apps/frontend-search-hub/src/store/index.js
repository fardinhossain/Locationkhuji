...
export const useLocationStore = create((set) => ({
  ...
  isNationwide: false,
  mapFocusTrigger: 0,
  setUser: (lat, lng) => set({ userLat: lat, userLng: lng, selectedLat: lat, selectedLng: lng, selectedName: "My Location", mapFocusTrigger: Date.now() }),
  setSelected: (lat, lng, name) => set((state) => ({ selectedLat: lat, selectedLng: lng, selectedName: name, mapFocusTrigger: state.mapFocusTrigger + 1 })),
  triggerFocus: () => set((state) => ({ mapFocusTrigger: state.mapFocusTrigger + 1 })),
